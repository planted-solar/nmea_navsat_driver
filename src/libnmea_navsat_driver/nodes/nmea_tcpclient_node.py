...
class NMEATCPClientNode(Node):
    def __init__(self):
        ...
        try:
            ...
            self.hardware_id = self.driver.declare_parameter(
                "hardware_id", "gnss_device"
            ).value
        ...
        self.get_logger().info(
            "Using gnss sensor with ip {} and port {}".format(
                self.gnss_ip, self.gnss_port
            )
        )
    ...
    def run(self):
        ...
        while rclpy.ok():
            try:
                ...
                stat.message, stat.level = (
                    "Socket Connection to (%s) Failed" % self.gnss_ip,
                    ...
                    for data in full_lines:
                        try:
                            if self.driver.add_sentence(data, self.frame_id):
                                gnss_stat.message, gnss_stat.level = (
                                    "GPS Fix",
                                    DiagnosticStatus.OK,
                                )
                                pass
                                ...
                                gnss_stat.message, gnss_stat.level = (
                                    "No GPS Fix: Error with sentence: %s" % data,
                            ...
                            gnss_stat.message, gnss_stat.level = (
                                "No GPS Fix: Value error: Error was: %s. Please report this issue. "
                                % e,
                                DiagnosticStatus.WARN,
                            )
                        self.publish_diagnostics([stat, gnss_stat])
